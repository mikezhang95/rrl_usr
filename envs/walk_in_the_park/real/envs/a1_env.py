...
class A1Real(gym.Env):
    ...
    def observation(self):
        ...
        # M: additional signals
        framequat = self.env._robot.GetBaseOrientation() # xyzw
        framequat = np.array([framequat[3], framequat[0], framequat[1], framequat[2]]).astype(np.float32) # wxyz
        gyro = imu[-3:]

        return np.concatenate(
             [qpos, qvel, self.prev_action, framequat, gyro, 
    ...
    def step(self, action):
        ...
        # M: current_action is  prev_action in obs
