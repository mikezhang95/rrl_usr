    ...
    if self._perturb_param == 'shin_length':
      ...
      if hasattr(self, "_perturb_all_list"):
        ...
        for part in parts:
          if part.get('class') == 'knee':
            part.find('./geom').set(
                'fromto', '0 0 0 {} 0 {}'.format(self._perturb_all_list[0],
                                                 -self._perturb_all_list[0]))
        ankles = mjcf.findall('./worldbody/body/body/body/body')
        for ankle in ankles:
          ankle.set('pos', '{} 0 {}'.format(self._perturb_all_list[0],
                                            -self._perturb_all_list[0]))
        torso_and_others = mjcf.findall('./worldbody/body/geom')
        for item in torso_and_others:
          if item.get('name') == 'torso':
            item.set('density', str(self._perturb_all_list[1]))
        joint_damping = mjcf.find('./default/default/joint')
        joint_damping.set('damping', str(self._perturb_all_list[2]))
        parts = mjcf.findall('./default/default/default')
        for part in parts:
          if part.get('class') == 'toe':
            part.find('./geom').set('friction', str(self._perturb_all_list[3]))
        floor_contact = mjcf.find('./worldbody/geom')  # Floor geom.
