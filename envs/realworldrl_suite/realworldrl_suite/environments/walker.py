...
class RealWorldPlanarWalker(realworld_env.Base, walker.PlanarWalker):
  ...
  def _setup_perturb(self, perturb_spec):
    ...
    if self._perturb_enabled:
      ...
      if self._perturb_param == 'thigh_length':
        ...
        self._perturb_max = perturb_spec.get('max', 2.)
        self._perturb_std = perturb_spec.get('std', 0.05)
        # only applies to constant perturbations
        value_list = perturb_spec.get('value_list', 1.)
        value_list = value_list.split('|')
        self._perturb_all_list = [float(v) for v in value_list]
  ...
  def update_physics(self):
    ...
    if self._perturb_param == 'thigh_length':
      ...
      if hasattr(self, "_perturb_all_list"):
        thighs = mjcf.findall('./worldbody/body/body/geom')
        for thigh in thighs:
          thigh.set('pos', '0 0 {}'.format(-self._perturb_all_list[0]))
          thigh.set('size', '0.05 {}'.format(self._perturb_all_list[0]))
        legs = mjcf.findall('./worldbody/body/body/body')
        for leg in legs:
          leg.set('pos', '0 0 {}'.format(-0.25 - 2 * self._perturb_all_list[0]))
        torso = mjcf.find('./worldbody/body/geom')
        torso.set('size', '0.07 {}'.format(self._perturb_all_list[1]))
        thighs = mjcf.findall('./worldbody/body/body')
        for thigh in thighs:
          thigh.set('pos', '0 -.05 {}'.format(-self._perturb_all_list[1]))
        joint_damping = mjcf.find('./default/joint')
        joint_damping.set('damping', str(self._perturb_all_list[2]))
        geom_contact = mjcf.find('./default/geom')
        geom_contact.set('friction', '{} .1 .1'.format(self._perturb_all_list[3]))
        floor_contact = mjcf.find('./worldbody/geom')  # Floor geom.
        floor_contact.set('friction', '{} .1 .1'.format(self._perturb_all_list[3]))
